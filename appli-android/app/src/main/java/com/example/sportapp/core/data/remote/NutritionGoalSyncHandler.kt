package com.example.sportapp.core.data.remote

import android.util.Log
import javax.inject.Inject
import javax.inject.Singleton

import org.json.JSONObject
import com.example.sportapp.core.data.local.NutritionGoalDao
import com.example.sportapp.core.data.model.NutritionGoal
import com.example.sportapp.core.utils.JsonUtils.getNullableString

@Singleton
class NutritionGoalSyncHandler @Inject constructor(
    private val dao: NutritionGoalDao
) {
    suspend fun handle(json: JSONObject) {
        when (json.getString("type")) {
            "nutrition_goal_updated" -> {
                val payload = json.getJSONObject("payload")
                Log.d("NutritionGoalHandler", "🆕 NutritionGoal update reçu: $payload")
                val entity = NutritionGoal(
                    uuid = payload.getString("uuid"),
                    userId = payload.getInt("userId"),
                    effectiveFrom = payload.getString("effectiveFrom"),
                    dayKind = payload.getString("dayKind"),
                    kcal = payload.getDouble("kcal").toFloat(),
                    proteinG = payload.getDouble("proteinG").toFloat(),
                    carbsG = payload.getDouble("carbsG").toFloat(),
                    fatG = payload.getDouble("fatG").toFloat(),

                    updatedAt = payload.getNullableString("updatedAt"),
                    synced = true
                )
                val existing = dao.getByUUID(entity.uuid)
                if (existing == null) dao.insertFromServer(entity) else dao.updateFromServer(entity)
            }
            "nutrition_goal_deleted" -> {
                val uuid = json.getString("uuid")
                Log.d("NutritionGoalHandler", "🗑️ NutritionGoal delete reçu: $uuid")
                val existing = dao.getByUUID(uuid)
                if (existing != null) {
                    dao.delete(existing)
                }
            }
        }
    }
}
